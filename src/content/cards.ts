...
export type CardKeyword = 'rush' | 'windfury' | 'flying' | 'stealth'
...
export const CARDS: Record<CardId, CardDefinition> = {
  ...
  burning_procession: s(
    ...
    3,
    ['fire'],
    '자신의 전장에 있는 불 몬스터 수만큼 자신의 덱 맨 위 카드를 확인한다. 그중 비용 1 이하인 불 몬스터를 원하는 만큼 소환할 수 있다. 나머지는 묘지로 보낸다.',
  ),
  ...
  overgrown_sprout: s(
    ...
    4,
    ['earth'],
    '자신의 마나에 땅 카드만 존재할 경우, 이번 턴 동안, 공격력이 1인 자신 몬스터가 직접 공격할 경우, 상대는 라이프를 하나 추가로 잃는다.',
  ),

  floating_mountains: u(
    'floating_mountains',
    '떠다니는 산맥',
    5,
    5,
