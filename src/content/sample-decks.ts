...
export const SAMPLE_DECKS: Record<SampleDeckId, SampleDeckDefinition> = {
  'fire-isolation-charge': {
    ...
    playGuide: '살아 움직이는 불꽃과 잿빛 들개로 초반 전장을 잡습니다. 화산쥐는 불 마나가 두 장 이상일 때 비용 없이 소환할 수 있으므로 중반 횡전개의 연결점으로 쓰고, 마지막 불씨는 가능하면 혼자 남겨 고립 돌진과 유언 드로우를 활용합니다. 불타는 행렬은 비용 2 이하 몬스터가 묘지로 간 뒤에도 전장을 다시 채우는 핵심 카드입니다.',
    manaPriority: '두 번째 이후의 불타는 행렬, 초반의 화산쥐 한 장, 지금 공격에 참여하기 어려운 저비용 몬스터. 화산쥐를 소환하려면 불 마나가 두 장 필요하다는 점을 먼저 맞춥니다.',
    testPoints: '화산쥐의 비용 0 전개 조건이 너무 쉽게 충족되지 않는지, 살아 움직이는 연기의 전투 공격력과 각성이 수비 효율을 과도하게 높이지 않는지 확인합니다.',
    cardIds: [
  ...
  'earth-mana-mountains': {
    ...
    playGuide: '씨 뿌리는 요정과 쓰나미 계열 효과로 덱 위 카드를 추가 마나로 만듭니다. 나무에 사는 요정이 마나에 놓이면 손의 카드 한 장을 추가로 준비 마나에 놓을 수 있어 성장 속도를 크게 당길 수 있습니다. 땅 마나가 네 장이 되면 너무 무거운 씨앗과 하늘까지 자라난 새싹이 활성화되고, 바위 갑옷 기사로 버틴 뒤 떠다니는 산맥의 질풍으로 마무리합니다.',
    manaPriority: '너무 무거운 씨앗, 두 번째 이후의 떠다니는 산맥, 당장 소환하기 어려운 고비용 카드. 나무에 사는 요정은 손의 다른 카드를 추가 마나로 바꿀 수 있으므로, 남길 카드와 마나로 보낼 카드를 함께 계산합니다.',
    testPoints: '나무에 사는 요정의 추가 마나가 손패를 지나치게 빠르게 소모하거나 마나 성장을 과도하게 가속하지 않는지, 너무 무거운 씨앗과 떠다니는 산맥이 지나치게 빠르게 이어지지 않는지 확인합니다.',
    cardIds: [
